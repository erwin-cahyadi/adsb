...
func displayAircrafts(l *List) {
	...
	fmt.Print(" ICAO   Flight   Altitude  Speed   Track   Latitude   Longitude   TTL\n")
	fmt.Print("========================================================================\n")
	for n := l.First(); n != nil; n = n.Next() {
		a = n.Aircraft
		stale_time := time.Since(n.Aircraft.seen)
		ttl := AIRCRAFT_TTL - stale_time.Nanoseconds() / 1000000
		fmt.Printf("%06X  %8s  %5d   %5d   %5d   %3.6f   %3.6f  %d\n", a.addr, a.flight, a.altitude, a.speed, a.track, a.lat, a.lon, ttl / 1000)
	}
	fmt.Printf("\n")	
}
...
type modesMessage struct {
	...
	velocity int64			/* Computed from EW and NS velocity */
...
func displayModesMessage(mm modesMessage) {
	...
	if(mm.msgtype == 0) {
		...
		if(mm.metype >= 1 && mm.metype <= 4) {
			...
			fmt.Printf("    Lat CPR         : %d (not decoded)\n", mm.raw_latitude)
			fmt.Printf("    Lon CPR         : %d (not decoded)\n", mm.raw_longitude)
			fmt.Printf("    Latitude        : %f\n", mm.latitude)
			fmt.Printf("    Longitude       : %f\n", mm.longitude)
		} else if(mm.metype >= 19 && mm.mesub >= 1 && mm.mesub <= 4) {
			if(mm.mesub == 1 || mm.mesub == 2) {
				fmt.Printf("    EW direction    : %d\n", mm.ew_dir)
				fmt.Printf("    EW velocity     : %d\n", mm.ew_velocity)
				fmt.Printf("    NS direction    : %d\n", mm.ns_dir)
				fmt.Printf("    NS velocity     : %d\n", mm.ns_velocity)
...
*/
	...
	for {
		...
		if(message[1] == 0x32 || message[1] == 0x33) {
			mm.timestamp = (uint64)(message[2])
			mm.timestamp = mm.timestamp << 8 + (uint64)(message[3])
			mm.timestamp = mm.timestamp << 8 + (uint64)(message[4])
			mm.timestamp = mm.timestamp << 8 + (uint64)(message[5])
			mm.timestamp = mm.timestamp << 8 + (uint64)(message[6])
			mm.timestamp = mm.timestamp << 8 + (uint64)(message[7])

			mm.time = mm.timestamp / 1024 / 1024 * 1000 / 8 / 128
			...
			if (mm.msgtype == 17) {
				...
				if (mm.metype >= 1 && mm.metype <= 4) {
					...
					if (mm.mesub == 1 || mm.mesub == 2) {
						...
						mm.vert_rate = (((int)(message[8]) & 7) << 6) | (((int)(message[9]) & 0xfc) >> 2)
						/* Compute velocity and angle from the two speed component */
						mm.velocity = (int64)(math.Sqrt(math.Pow((float64)(mm.ns_velocity), 2) + math.Pow((float64)(mm.ew_velocity), 2)))
						if (mm.velocity != 0) {
							ewv := mm.ew_velocity
							nsv := mm.ns_velocity
						...
						mm.heading = (360 / 128) * (((int)(message[5]) & 3 << 5) | ((int)(message[6]) >> 3))
					}
				}
			}
